...
public class MeasurementFileReader : ICanProvideMeasurements
{
    ...
    private static readonly Regex NoiseRegex = new Regex(
        @"Beschwerde zu (\d{2}:\d{2}:\d{2}) Uhr versenden \[(\d+\.\d+) dBA",
    ...
    public async Task<
        IEnumerable<Domain.NoiseMeasurement>
    > GetNoiseMeasurementsForPastTimePeriodAsync(DateTime endTimeUtc, TimeSpan duration)
    {
    ...
    private List<NoiseMeasurement> ParseHtmlData(string htmlContent)
    {
        ...
        var areaElements = doc.DocumentNode.SelectNodes(
            "//area[contains(@title, 'Beschwerde zu')]"
        );
